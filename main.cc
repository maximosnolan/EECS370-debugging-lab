...
//convert a vector of strings into a singular string seperated by spaces only IF all strings passed in are non empty
    [[nodiscard]] auto accumulate(std::vector<std::string> & vec) -> std::optional<std::string> {
    ...
    //define lambda expression for the accumulation
        ...
        std::string blank = "";
        auto returnValue = std::accumulate(vec.begin(), vec.end(), blank,  add_space_fold);
...
[[nodiscard]] int main(int argc, char ** argv) {
    ...
    std::optional<std::string> value = example::accumulate(vec);
    // if (value.has_value()) {
